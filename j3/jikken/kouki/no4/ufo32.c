     ...
     /* 但し, きれいに関数化しても差障りはないかもしれない      */
{
  ...
  int current_stage = 1;
  short tama, teki=3, prev_teki = teki;

  char info_disp[] = "S:   E:   B:    ";

//this macro cant be used as an expression
#define SETUP_VARIABLES()\
  teki = prev_teki + random() % (current_stage*2);\
  tama = teki + 99/current_stage;\
  prev_teki = teki;\
  target = 0;\
  t = '0';\
  disp[0] = '0'; disp[1] = ':';\ 
  for (i = 2; i < MAXINVNUM; i++) disp[i] = ' ';\
  disp[MAXINVNUM] = '\0';\
  info_disp[12] = tama >= 99 ? 99 : tama / 10 + '0';	\
  info_disp[13] = tama >= 99 ? 99 : tama % 10 + '0';		\
  info_disp[7] = teki >= 99 ? 99 : teki / 10 + '0';		\
  info_disp[8] = teki >= 99 ? 99 : teki % 10 + '0';\
  info_disp[2] = current_stage / 10 + '0';\
  info_disp[3] = current_stage % 10 + '0';\

  tama = teki + teki;
  info_disp[12] = tama / 10 + '0';
  info_disp[13] = tama % 10 + '0';     
  info_disp[7] = teki / 10 + '0';
  info_disp[8] = teki % 10 + '0';
  info_disp[2] = '0';
  info_disp[3] = '1';

  lcd_cursor(0, 0);
  lcd_printstr(info_disp);
